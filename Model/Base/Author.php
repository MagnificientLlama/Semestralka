...
abstract class Author implements ActiveRecordInterface
{
    ...
    public function setAuthid($v)
    {
        ...
        if ($this->authid !== $v) {
            $this->authid = $v;
            $this->modifiedColumns[AuthorTableMap::COL_AUTHID] = true;
        }
    ...
    public function setAuthname($v)
    {
        ...
        if ($this->authname !== $v) {
            $this->authname = $v;
            $this->modifiedColumns[AuthorTableMap::COL_AUTHNAME] = true;
        }
    ...
    public function setDateofbirth($v)
    {
        ...
        if ($this->dateofbirth !== null || $dt !== null) {
            if ($this->dateofbirth === null || $dt === null || $dt->format("Y-m-d") !== $this->dateofbirth->format("Y-m-d")) {
                ...
                $this->modifiedColumns[AuthorTableMap::COL_DATEOFBIRTH] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AuthorTableMap::translateFieldName('Authid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->authid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AuthorTableMap::translateFieldName('Authname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->authname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AuthorTableMap::translateFieldName('Dateofbirth', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[AuthorTableMap::COL_AUTHID] = true;
        if (null !== $this->authid) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . AuthorTableMap::COL_AUTHID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(AuthorTableMap::COL_AUTHID)) {
            $modifiedColumns[':p' . $index++]  = 'authID';
        }
        if ($this->isColumnModified(AuthorTableMap::COL_AUTHNAME)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'authID':
                        $stmt->bindValue($identifier, $this->authid, PDO::PARAM_INT);
                        break;
                    case 'authName':
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setAuthid($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setAuthname($arr[$keys[1]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::COL_AUTHID)) {
            $criteria->add(AuthorTableMap::COL_AUTHID, $this->authid);
        }
        if ($this->isColumnModified(AuthorTableMap::COL_AUTHNAME)) {
            $criteria->add(AuthorTableMap::COL_AUTHNAME, $this->authname);
        }
        if ($this->isColumnModified(AuthorTableMap::COL_DATEOFBIRTH)) {
    ...
    /**
     * Generic method to set the primary key (authid column).
     *
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setAuthid(NULL); // this is a auto-increment column, so set to default value
        }
    }
