...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * The value for the privilegy field.
     *
    ...
    /**
     * The value for the useravatar field.
     *
     * @var        string
     */
    ...
    /**
     * @var        ObjectCollection|ChildReadinglist[] Collection to store aggregation of ChildReadinglist objects.
     */
    protected $collReadinglists;
    protected $collReadinglistsPartial;
     ...
     * @var ObjectCollection|ChildReadinglist[]
     */
    protected $readinglistsScheduledForDeletion = null;
    ...
    /**
     * Get the [privilegy] column value.
     *
     * @return int
     */
    ...
    /**
     * Get the [useravatar] column value.
     *
     * @return string
     */
    ...
    /**
     * Set the value of [privilegy] column.
     *
     * @param int $v new value
     * @return $this|\User The current object (for fluent API support)
     */
    public function setPrivilegy($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if ($this->privilegy !== $v) {
            $this->privilegy = $v;
            $this->modifiedColumns[UserTableMap::COL_PRIVILEGY] = true;
        }
    ...
    /**
     * Set the value of [useravatar] column.
     *
     * @param string $v new value
     * @return $this|\User The current object (for fluent API support)
     */
    public function setUseravatar($v)
    {
        ...
        if ($this->useravatar !== $v) {
            $this->useravatar = $v;
            $this->modifiedColumns[UserTableMap::COL_USERAVATAR] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('Privilegy', TableMap::TYPE_PHPNAME, $indexType)];
            $this->privilegy = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('Useravatar', TableMap::TYPE_PHPNAME, $indexType)];
            $this->useravatar = (null !== $col) ? (string) $col : null;
            $this->resetModified();
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->readinglistsScheduledForDeletion !== null) {
                if (!$this->readinglistsScheduledForDeletion->isEmpty()) {
                    \ReadinglistQuery::create()
                        ->filterByPrimaryKeys($this->readinglistsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->readinglistsScheduledForDeletion = null;
                }
            }

            if ($this->collReadinglists !== null) {
                foreach ($this->collReadinglists as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PRIVILEGY)) {
            $modifiedColumns[':p' . $index++]  = 'privilegy';
        }
        if ($this->isColumnModified(UserTableMap::COL_USERAVATAR)) {
            $modifiedColumns[':p' . $index++]  = 'userAvatar';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'password':
                        ...
                        break;
                    case 'privilegy':
                        $stmt->bindValue($identifier, $this->privilegy, PDO::PARAM_INT);
                        break;
                    case 'userAvatar':
                        $stmt->bindValue($identifier, $this->useravatar, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getPrivilegy(),
            $keys[5] => $this->getUseravatar(),
        );
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collReadinglists) {
                ...
                $result[$key] = $this->collReadinglists->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setPrivilegy($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setUseravatar($arr[$keys[5]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PRIVILEGY)) {
            $criteria->add(UserTableMap::COL_PRIVILEGY, $this->privilegy);
        }
        if ($this->isColumnModified(UserTableMap::COL_USERAVATAR)) {
            $criteria->add(UserTableMap::COL_USERAVATAR, $this->useravatar);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setPrivilegy($this->getPrivilegy());
        $copyObj->setUseravatar($this->getUseravatar());
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getReadinglists() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addReadinglist($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('Readinglist' == $relationName) {
            $this->initReadinglists();
            return;
        }
    ...
    /**
     * Clears out the collReadinglists collection
     *
     ...
     * @see        addReadinglists()
     */
    public function clearReadinglists()
    {
        $this->collReadinglists = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collReadinglists collection loaded partially.
     */
    public function resetPartialReadinglists($v = true)
    {
        $this->collReadinglistsPartial = $v;
    }

    /**
     * Initializes the collReadinglists collection.
     *
     * By default this just sets the collReadinglists collection to an empty array (like clearcollReadinglists());
     ...
     * @return void
     */
    public function initReadinglists($overrideExisting = true)
    {
        if (null !== $this->collReadinglists && !$overrideExisting) {
            return;
        }

        $collectionClassName = ReadinglistTableMap::getTableMap()->getCollectionClassName();

        $this->collReadinglists = new $collectionClassName;
        $this->collReadinglists->setModel('\Readinglist');
    }

    /**
     * Gets an array of ChildReadinglist objects which contain a foreign key that references this object.
     *
     ...
     * @return ObjectCollection|ChildReadinglist[] List of ChildReadinglist objects
     * @throws PropelException
     */
    public function getReadinglists(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collReadinglistsPartial && !$this->isNew();
        if (null === $this->collReadinglists || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collReadinglists) {
                // return empty collection
                $this->initReadinglists();
            } else {
                $collReadinglists = ChildReadinglistQuery::create(null, $criteria)
                    ->filterByUser($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collReadinglistsPartial && count($collReadinglists)) {
                        $this->initReadinglists(false);

                        foreach ($collReadinglists as $obj) {
                            if (false == $this->collReadinglists->contains($obj)) {
                                $this->collReadinglists->append($obj);
                            }
                        }

                        $this->collReadinglistsPartial = true;
                    }

                    return $collReadinglists;
                }

                if ($partial && $this->collReadinglists) {
                    foreach ($this->collReadinglists as $obj) {
                        if ($obj->isNew()) {
                            $collReadinglists[] = $obj;
                        }
                    }
                }

                $this->collReadinglists = $collReadinglists;
                $this->collReadinglistsPartial = false;
            }
        }

        return $this->collReadinglists;
    }

    /**
     * Sets a collection of ChildReadinglist objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $readinglists A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildUser The current object (for fluent API support)
     */
    public function setReadinglists(Collection $readinglists, ConnectionInterface $con = null)
    {
        /** @var ChildReadinglist[] $readinglistsToDelete */
        $readinglistsToDelete = $this->getReadinglists(new Criteria(), $con)->diff($readinglists);


        $this->readinglistsScheduledForDeletion = $readinglistsToDelete;

        foreach ($readinglistsToDelete as $readinglistRemoved) {
            $readinglistRemoved->setUser(null);
        }

        $this->collReadinglists = null;
        foreach ($readinglists as $readinglist) {
            $this->addReadinglist($readinglist);
        }

        $this->collReadinglists = $readinglists;
        $this->collReadinglistsPartial = false;
    ...
    /**
     * Returns the number of related Readinglist objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Readinglist objects.
     * @throws PropelException
     */
    public function countReadinglists(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collReadinglistsPartial && !$this->isNew();
        if (null === $this->collReadinglists || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collReadinglists) {
            ...
            if ($partial && !$criteria) {
                return count($this->getReadinglists());
            }

            $query = ChildReadinglistQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collReadinglists);
    }

    /**
     * Method called to associate a ChildReadinglist object to this object
     * through the ChildReadinglist foreign key attribute.
     *
     * @param  ChildReadinglist $l ChildReadinglist
     * @return $this|\User The current object (for fluent API support)
     */
    public function addReadinglist(ChildReadinglist $l)
    {
        if ($this->collReadinglists === null) {
            $this->initReadinglists();
            $this->collReadinglistsPartial = true;
        ...
        if (!$this->collReadinglists->contains($l)) {
            $this->doAddReadinglist($l);

            if ($this->readinglistsScheduledForDeletion and $this->readinglistsScheduledForDeletion->contains($l)) {
                $this->readinglistsScheduledForDeletion->remove($this->readinglistsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildReadinglist $readinglist The ChildReadinglist object to add.
     */
    protected function doAddReadinglist(ChildReadinglist $readinglist)
    {
        $this->collReadinglists[]= $readinglist;
        $readinglist->setUser($this);
    }

    /**
     * @param  ChildReadinglist $readinglist The ChildReadinglist object to remove.
     * @return $this|ChildUser The current object (for fluent API support)
     */
    public function removeReadinglist(ChildReadinglist $readinglist)
    {
        if ($this->getReadinglists()->contains($readinglist)) {
            $pos = $this->collReadinglists->search($readinglist);
            $this->collReadinglists->remove($pos);
            if (null === $this->readinglistsScheduledForDeletion) {
                $this->readinglistsScheduledForDeletion = clone $this->collReadinglists;
                $this->readinglistsScheduledForDeletion->clear();
            }
            $this->readinglistsScheduledForDeletion[]= clone $readinglist;
    ...
    public function setUserratings(Collection $userratings, ConnectionInterface $con = null)
    {
        ...
        foreach ($userratingsToDelete as $userratingRemoved) {
            $userratingRemoved->setUser(null);
        }
     ...
     * @return ObjectCollection|ChildUserrating[] List of ChildUserrating objects
     */
    public function getUserratingsJoinBook(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUserratingQuery::create(null, $criteria);
    ...
    {
        if ($deep) {
            if ($this->collReadinglists) {
                foreach ($this->collReadinglists as $o) {
